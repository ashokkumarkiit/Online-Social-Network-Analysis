...
"""
Fetched labelled data and tweets from excel file labelled manually
"""
def get_labeled_data(loc_type): 
    data = []
    # Give the location of the excel file 
    location = ("./manually_labelled/" + loc_type + "/tweets.xlsx") 
    ...
    for i in range(0,sheet.nrows):
        data += [(int(sheet.cell_value(i, 2)) ,str(sheet.cell_value(i, 1) ))]
    return np.array([d[1] for d in data]), np.array([d[0] for d in data])

"""
This method is used for removing occurance of stop words from the text.
"""
...
def getAFINNTerms():
    """
    This methid pulls the library of AFINN and loads into the variable for validation.
    """
...
def parse_test_data(best_result, vocab):
    ...
    ###TODO
    tokens_list = []
    #test_docs, test_labels = label_tweets(users,'./tweets/test/','tweets.pkl')
...
def print_top_predicted(test_docs, test_labels, X_test, clf, n, predicted_type, predicted_value = 0):
    """
    ...
    Params:
      ...
      predicted_type..... 'misclassified' is used for misclassified documents
                          'actual' is used for actual positive and negative documents
      predicted_value.... 0 for negative documents
                          1 for positive documents
    ...
    for index,value in enumerate(clf.predict(X_test)):
        if(predicted_type == 'misclassified'):
            ...
            if(predicted_values[index] == test_labels[index] and predicted_values[index] == predicted_value):
                misclassified_element = {}
                misclassified_element["truth"] = test_labels[index]
                misclassified_element["predicted"] = predicted_values[index]
                misclassified_element["proba"] = predicted_probability[index][value]
                ...
                temp.append(predicted_probability[index][value])
                top_misclassified_list[index]=(misclassified_element)
    for document in sorted(list(top_misclassified_list.items()), key=lambda x:-x[1]['proba'])[:n]:
...
def main():
    ...
    misclassified_list = print_top_predicted(test_docs, test_labels, X_test, clf, 5, 'misclassified')
    pickle.dump(misclassified_list, open('./misclassified_list.pkl','wb'))

    print('\nTOP NEGATIVE TEST DOCUMENTS:')
    negative_predicted_list = print_top_predicted(test_docs, test_labels, X_test, clf, 5, 'actual', 0)
    pickle.dump(negative_predicted_list, open('./negative_predicted_list.pkl','wb'))

    print('\nTOP POSITIVE TEST DOCUMENTS:')
    positive_predicted_list = print_top_predicted(test_docs, test_labels, X_test, clf, 5, 'actual', 1)
    pickle.dump(positive_predicted_list, open('./positive_predicted_list.pkl','wb'))
