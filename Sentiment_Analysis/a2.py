...
def lexicon_features(tokens, feats):
    ...
    """
    ###TODO
    neg_words_lower = set([ x.lower() for x in  neg_words ])
    pos_words_lower = set([ x.lower() for x in  pos_words ])
    feats["neg_words"] = 0
    feats["pos_words"] = 0
    for item in tokens:
        if(item.lower() in neg_words_lower):
            feats["neg_words"] += 1
        elif(item.lower() in pos_words_lower):
            feats["pos_words"] += 1
